...
Facem o clasa test, care mosteneste TestCase
Implementam  metodele necesare:
    setUp-  ce se intampla inainte de fiecare test
    tearDown - ce se intampla dupa fiecare test

Scriem oricare metode de test dorim(IMPORTANT: numele acestora trebuie sa inceapa cu test)
...
class JulesTestCase(unittest.TestCase):
    ...
    def test_forgot_password_redirect(self):
        ...
        self.assertEqual(self.chrome.current_url, expected_url, "URl-ul nu este cel dorit")
...
#         time.sleep(2)
#
#         cart_page_tile = self.driver.find_element(By.XPATH, '//div[@class="title"]')
#         self.assertEqual(cart_page_tile.text, 'Cosul tau este gol')